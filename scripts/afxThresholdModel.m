function thresholdMaps = afxThresholdModel(x,y,idxTrain,design,space,masks)
    ...
    design.predictors{end+1} = 'CBVRel';
    ...
    for iThreshold = 1:length(design.thresholdMaps)
        ...
        thresholdMaps(iThreshold).optThr = afxOptimalThreshold(x(idxOptThr,idxThr,:),y(idxOptThr,:),.001,design.thresholdMaps(iThreshold).inverse);
